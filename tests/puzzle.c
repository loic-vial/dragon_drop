...
static inline ei_bool_t valid(puzzle_t * puzzle, ei_point_t position)
{
	return (position.x >= 0) &&
	    ...
	    (position.y >= 0) && (position.y < puzzle->n.height);
}
...
static inline tile_t *tile_memory_at(puzzle_t * puzzle,
				     ei_point_t position)
{
...
ei_bool_t handle_tile_press(ei_widget_t * widget, ei_event_t * event,
			    void *user_param)
{
	...
	for (i = 0; i < 4; i++) {
		...
		if (valid(puzzle, swap_pos)
		    && tile_at(puzzle, swap_pos) == NULL) {
			...
			ei_place(tile->button, NULL, &(swap_coordinates.x),
				 &(swap_coordinates.y), NULL, NULL, NULL,
				 NULL, NULL, NULL);
		}
	}
...
void create_puzzle_window(char *image_filename)
{
	...
	n = ei_size(image_size.width / k_tile_size,
		    image_size.height / k_tile_size);

	toplevel = ei_widget_create("toplevel", ei_app_root_widget());
	toplevel_size =
	    ei_size(n.width * k_tile_size, n.height * k_tile_size);
	border_width = 0;
	ei_toplevel_configure(toplevel, &toplevel_size, &toplevel_bg,
			      &border_width, &title, NULL, &resizable,
			      NULL);
	ei_place(toplevel, NULL, &(g_toplevel_position.x),
		 &(g_toplevel_position.y), NULL, NULL, NULL, NULL, NULL,
		 NULL);
	g_toplevel_position =
	    ei_point_add(g_toplevel_position, ei_point(12, 12));

	puzzle = (puzzle_t *) malloc(sizeof(*puzzle));
	puzzle->n = n;
	puzzle->tiles =
	    (tile_t *) malloc(n.width * n.height * sizeof(tile_t));
	puzzle->current =
	    (tile_t **) malloc(n.width * n.height * sizeof(tile_t *));

	border_width = 2;
	image_rect_size =
	    ei_size_sub(tile_size,
			ei_size(2 * border_width, 2 * border_width));
	...
	for (y = 0; y < n.height; y++) {
		for (x = 0; x < n.width; x++) {
			...
			img_rect =
			    ei_rect(ei_point
				    (x * k_tile_size + border_width,
				     y * k_tile_size + border_width),
				    image_rect_size);
			tile = tile_memory_at(puzzle, ei_point(x, y));
			ei_button_configure(button, &tile_size, &grey,
					    &border_width, &corner_radius,
					    &relief, NULL, NULL, NULL,
					    NULL, &image, &img_rect_ptr,
					    NULL, &callback,
					    (void *) &tile);
			place_coords = place_coordinates(current_position);
			ei_place(button, NULL, &(place_coords.x),
				 &(place_coords.y), NULL, NULL, NULL, NULL,
				 NULL, NULL);
			...
			puzzle->
			    current[index_at(puzzle, current_position)] =
			    tile;
		}
	}
}
...
ei_bool_t handle_keydown(ei_widget_t * widget, ei_event_t * event,
			 void *user_param)
{
	uint32_t cmd_mask =
	    (1 << ei_mod_ctrl_left) | (1 << ei_mod_ctrl_right) | (1 <<
								  ei_mod_meta_left)
	    | (1 << ei_mod_meta_right);
