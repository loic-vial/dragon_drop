	     ...
	     ei_tag_t tag, ei_callback_t callback, void *user_param)
{
	...
	if (widget != NULL && tag != NULL)
		return;
	ei_eventlist_t *event =
	    (ei_eventlist_t *) malloc(sizeof(ei_eventlist_t));
	...
	if (first_event == NULL) {
		...
		ei_eventlist_t *tmp;
		for (tmp = first_event; tmp->next != NULL;
		     tmp = tmp->next);
		event->previous = tmp;
		tmp->next = event;
	}
...
void ei_unbind(ei_eventtype_t eventtype, ei_widget_t * widget,
	       ei_tag_t tag, ei_callback_t callback, void *user_param)
{
	...
	while (event != NULL) {
		...
		if (event->eventtype == eventtype &&
		    ...
		    ((tag != NULL && event->tag != NULL
		      && strcmp(event->tag, tag) == 0) || (tag == NULL
							   && event->tag ==
							   NULL))
		    && event->callback == callback
		    && event->user_param == user_param) {
