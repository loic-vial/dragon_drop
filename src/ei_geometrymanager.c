...
void ei_geometrymanager_register(ei_geometrymanager_t * geometrymanager)
{
	if (first_manager == NULL) {
		...
		for (tmp = first_manager; tmp->next != NULL;
		     tmp = tmp->next);
...
{
	for (ei_geometrymanager_t * tmp = first_manager; tmp != NULL;
	     tmp = tmp->next) {
...
void ei_register_placer_manager()
{
	ei_geometrymanager_t *placer =
	    (ei_geometrymanager_t *) malloc(sizeof(ei_geometrymanager_t));
	      ...
	      float *rel_height)
{
	if (widget != NULL) {
		...
		if (widget->geom_params == NULL) {
			place = (ei_placer_geometry_param_t *)
			    malloc(sizeof(ei_placer_geometry_param_t));
			place->geometry_param.manager =
			    ei_geometrymanager_from_name("placer");
			widget->geom_params =
			    (ei_geometry_param_t *) place;
		} else
		    if (strcmp
			(widget->geom_params->manager->name,
			 "placer") != 0) {
