...
void ei_widgetclass_register(ei_widgetclass_t * widgetclass)
{
	if (first_class == NULL) {
		...
		for (tmp = first_class; tmp->next != NULL;
		     tmp = tmp->next);
...
{
	for (ei_widgetclass_t * tmp = first_class; tmp != NULL;
	     tmp = tmp->next) {
...
void ei_button_register_class()
{
	...
	ei_bind(ei_ev_mouse_buttondown, NULL, "button",
		ei_button_click_down, NULL);
	ei_bind(ei_ev_mouse_buttonup, NULL, "button", ei_button_click_up,
		NULL);
}

void ei_toplevel_register_class()
{
	ei_widgetclass_t *toplevelclass =
	    (ei_widgetclass_t *) malloc(sizeof(ei_widgetclass_t));
