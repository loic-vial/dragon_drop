...
ei_widget_t *ei_widget_create(ei_widgetclass_name_t class_name,
			      ei_widget_t * parent)
{
	ei_widgetclass_t *widgetclass =
	    ei_widgetclass_from_name(class_name);
...
ei_widget_t *ei_widget_pick(ei_point_t * where)
{
	...
	buffer =
	    &buffer[(where->x +
		     where->y * size.width) * sizeof(ei_color_t)];
	int ir, ig, ib, ia;
	hw_surface_get_channel_indices(offscreen_surface, &ir, &ig, &ib,
				       &ia);
	ei_color_t color =
	...
	if (frame->img != NULL) {
		if (frame->img_rect == NULL) {
			...
			widget->requested_size.width =
			    ...
			    width ? size.width : widget->requested_size.
			    width;
			widget->requested_size.height =
			    size.height >
			    widget->requested_size.
			    height ? size.height : widget->requested_size.
			    height;
		} else {
			widget->requested_size.width =
			    frame->img_rect->size.width >
			    widget->requested_size.
			    width ? frame->img_rect->size.width : widget->
			    requested_size.width;
			widget->requested_size.height =
			    frame->img_rect->size.height >
			    widget->requested_size.
			    height ? frame->img_rect->size.
			    height : widget->requested_size.height;
		}
	} else if (frame->text != NULL) {
		int width, height;
		hw_text_compute_size(frame->text, frame->text_font, &width,
				     &height);
		...
		widget->requested_size.width =
		    width >
		    widget->requested_size.
		    width ? width : widget->requested_size.width;
		widget->requested_size.height =
		    height >
		    widget->requested_size.
		    height ? height : widget->requested_size.height;
	}
}

void ei_frame_configure(ei_widget_t * widget, ei_size_t * requested_size,
			const ei_color_t * color, int *border_width,
			ei_relief_t * relief, char **text,
			...
			ei_rect_t ** img_rect, ei_anchor_t * img_anchor)
{
	...
	if (img_rect != NULL) {
		...
		if (*img_rect != NULL) {
			frame->img_rect =
			    (ei_rect_t *) malloc(sizeof(ei_rect_t));
			 ...
			 ei_callback_t * callback, void **user_param)
{
	...
	ei_frame_configure(widget, requested_size, color, border_width,
			   relief, text, text_font, text_color,
	...
	if (user_param != NULL || callback != NULL) {
		ei_unbind(ei_ev_mouse_buttonup, widget, NULL,
			  button->callback, button->user_param);
		...
		ei_bind(ei_ev_mouse_buttonup, widget, NULL,
			button->callback, button->user_param);
	}
}

void ei_toplevel_configure(ei_widget_t * widget,
			   ei_size_t * requested_size, ei_color_t * color,
			   int *border_width, char **title,
			   ...
			   ei_size_t ** min_size)
{
	...
	if (closable != NULL) {
		...
		if (toplevel->closable) {
			...
			ei_place(&toplevel->close_button->frame.widget,
				 &resize_button_anchor, &resize_button_x,
				 NULL, NULL, NULL, NULL, NULL, NULL, NULL);
		} else {
			ei_geometrymanager_unmap(&toplevel->
						 close_button->frame.
						 widget);
		}
	}
	...
	if (resizable != NULL) {
		...
		if (toplevel->resizable != ei_axis_none) {
			ei_anchor_t resize_button_anchor =
			    ei_anc_southeast;
			ei_place(&toplevel->resize_button->frame.widget,
				 &resize_button_anchor, NULL, NULL, NULL,
	...
	if (min_size != NULL) {
		...
		if (*min_size != NULL) {
			toplevel->min_size =
			    (ei_size_t *) malloc(sizeof(ei_size_t));
