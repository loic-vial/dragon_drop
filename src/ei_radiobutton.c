...
ei_bool_t close_button_click2(ei_widget_t* widget, ei_event_t* event, void* user_param)
{
...
void add_children_radiobutton(char * text, ei_radiobutton_t* radio)
{
    ...
    close_button->callback = close_button_click2;
    ei_bind(ei_ev_mouse_buttonup, &close_button->frame.widget, NULL, close_button_click2, NULL);
...
void setdefaultsfunc_radiobutton(ei_widget_t* widget)
{
    ...
    add_children_radiobutton("premier champ",radio);
}
...
void ei_destroy_field(ei_radiobutton_t* radio,int number)
{
    ...
    if (widget !=NULL)
    {
        ...
        radio->number--;
        radio->frame.widget.requested_size.height-=20;
        while(tmp!=NULL)
        {
            ...
            point.x=0;
            point.y=tmp->screen_location.top_left.y-20;
...
void ei_edit_field(ei_radiobutton_t* radio,int number)
{
    ei_widget_t* widget;
    widget= ei_find_widget( radio, number);
}
