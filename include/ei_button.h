...
/**
 * \brief	A type of widget which represent a button
 */
typedef struct ei_button_t {
	ei_frame_t frame;	///< Frame to memorized all common fields between button and frame. All fields of the radiobutton will be children of this frame
	ei_callback_t callback;	///< This function will be called then the button is clicked
	void *user_param;	///< Parameter send by the user
} ei_button_t;

/**
 * @brief	Allocates a block of the size of a \ref ei_button_t in the memory.
...
/**
 * @brief Callback called when a button is down.
 ...
 * @return
 */
ei_bool_t ei_button_click_down(ei_widget_t * widget, ei_event_t * event,
			       void *user_param);

/**
 * @brief Callback called then a clicked button become up
